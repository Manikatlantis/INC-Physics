...
routes predict criticality. Calibrating the L=8 learned-\(\ln Z\) peak by the
6.5% known-truth miss at L=4 gives \(T_c(\log Z)=2.34\pm0.15\). The observable
route remains two separate estimates: susceptibility extrapolation gives 2.274
and the Binder-crossing mean gives 2.246, a range of about 2.25 to 2.27. Their
finite-size biases have opposite signs, so averaging them would overstate
precision. The exact thermodynamic-limit value is 2.2692. These results show that
a small trajectory-balance model can learn both samples and normalization, but
they also expose derivative sensitivity, finite-size bias, and places where
...
[NOTES.md](NOTES.md). The experiments and original report build ran in
one automated session of roughly 90 minutes wall-clock.
...
Each empirical distribution below uses two million seeded
multinomial draws from exactly enumerated model probabilities and a disclosed
Jeffreys half-count for bins absent from the finite histogram. For this
normalized autoregressive model, those draws are distributionally equivalent to
sequential rollout terminals, but they were not rollouts. The unsmoothed
enumerated-model KL is also shown. M3 and M4 sampling, by contrast, used true
sequential rollouts.
...
For L=8, each comparison uses 200,000 GFlowNet terminals generated by true
sequential rollouts and 256,000 fresh Metropolis samples:
...
This validates the differentiation pipeline on exact data; it does not validate
the learned \(\ln Z\) values or their curvature. Applied unchanged to the learned
L=4 function, the peak is T=2.281360, 6.5% below the exact-logZ peak. Treating
that known-truth miss as an empirical relative calibration error on the raw L=8
central peak gives the reported logZ-route result:

\[
\boxed{T_c^{(\log Z)}=2.34\pm0.15}
\]

In plain-text form, **Tc(logZ) = 2.34 +/- 0.15**. The central value lies inside
the required [2.1, 2.5] finite-size window, and the exact thermodynamic-limit Tc
lies inside the empirical error bar. This is a calibration error bar, not a
statistical confidence interval: the differentiation pipeline was validated on
exact data, but the learned logZ itself carries this calibration error
[M4 JSON](results/m4_metrics_20260818T021212-0400.json).

### 5.5 Critical temperature from generated observables

M4 used true sequential rollouts for the GFlowNet curves. Local quadratic fits
give susceptibility peaks at T=2.812028 (L=4 GFlowNet), 2.552567 (L=8
GFlowNet), and 2.447225 (L=12 Metropolis). Fitting
...
As a methodological control, the same local-quadratic and \(1/L\) procedure was
applied to the existing M1 Metropolis-only curves at L=4, 8, and 12. Their peaks
are 2.830413, 2.550983, and 2.446206, giving

\[
T_c^{(\chi,\mathrm{M1\ MCMC})}=2.259472,\qquad R^2=0.99941.
\]

No new sampling was performed. The M1 temperature grid is coarser around the
L=4 and L=8 maxima; changing the local fit from five to three points moves the
control intercept by about 0.008. It is therefore useful as a method control,
not as a higher-precision replacement for the mixed fit
[M1 JSON](results/m1_metrics_20260818T002648-0400.json).

Binder crossings are T=2.242534 for L4/L8 and T=2.249908 for L8/L12; their mean
is 2.246221. The two reported observable estimates are therefore the mixed
susceptibility extrapolation **2.274** and Binder mean **2.246**, an
observable-route range of about **2.25 to 2.27**. Relative to exact Tc, the
susceptibility estimate is 0.19% high and the Binder mean is 1.01% low. These
finite-size biases have opposite signs, so averaging them overstates precision;
no observable consensus is reported
[M4 JSON](results/m4_metrics_20260818T021212-0400.json).
...
sampling. The independent observable estimates then span Onsager's value using
generated L=4/L=8 states plus an L=12 MCMC anchor.
...
- **Derivative sensitivity.** Small value errors in learned \(\ln Z\) are
  ...
  physical discovery. The exact-data pipeline test is accurate, but the learned
  function's L=4 miss motivates the reported 6.5% empirical calibration bar.
...
- **Uncertainty.** Validators use large seeded samples and independent chains,
  ...
  uncertainty bands. The logZ bar is empirical calibration, not a confidence
  interval, and the reported digits describe this reproducible run rather than
...
an independent Metropolis baseline. Its calibrated partition-function route
gives Tc(logZ) = 2.34 +/- 0.15. The generated-observable route is reported as
separate susceptibility and Binder estimates, 2.274 and 2.246, spanning about
2.25 to 2.27 around the exact 2.2692; their opposite-sign biases are not averaged
into a falsely precise consensus. The logZ result is the more distinctive
model-based measurement and exposes the greater numerical fragility of
thermodynamic derivatives. Within the stated small-lattice and CPU limits, the
project meets its goal without treating known-physics contradictions as new
discoveries.
